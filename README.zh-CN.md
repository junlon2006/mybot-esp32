...
| 输入 | CST9217 INT GPIO11；Boot GPIO0 |

| Profile | 音频 MCLK | CO5300 reset | CST9217 reset | TCA9554 |
| --- | --- | --- | --- | --- |
...
资料也存在冲突。扩展分区前必须先通过启动日志确认实际 Flash 容量；当前保守配置可安全用于
两种容量。
...
明暗主题和可选的 5 fps 状态动画通过 menuconfig 选择；关闭状态活动动画后仍保留必要的
配网文字滚动，触摸手势保持不变。
