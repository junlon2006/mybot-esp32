 ...
 */
/* Modified for local vendoring: formatting normalized and ID helpers follow Kconfig. */
...
static bool esp_lcd_touch_cst816s_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y,
                                         uint16_t *strength, uint8_t *point_num,
                                         uint8_t max_point_num);
static esp_err_t esp_lcd_touch_cst816s_del(esp_lcd_touch_handle_t tp);

static esp_err_t touch_cst816s_i2c_read(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data,
                                        uint8_t len);
...
esp_err_t esp_lcd_touch_new_i2c_cst816s(const esp_lcd_panel_io_handle_t io,
    ...
    ESP_RETURN_ON_FALSE(io != NULL, ESP_ERR_INVALID_ARG, TAG,
                        "Touch controller io handle can't be NULL");
...
static esp_err_t esp_lcd_touch_cst816s_read_data(esp_lcd_touch_handle_t tp) {
    ESP_RETURN_ON_FALSE(tp != NULL, ESP_ERR_INVALID_ARG, TAG,
                        "Touch controller handle can't be NULL");
    ...
    data_t point;
    ESP_RETURN_ON_ERROR(
        touch_cst816s_i2c_read(tp, DATA_START_REG, (uint8_t *)&point, sizeof(data_t)), TAG,
        "I2C read failed");
...
static bool esp_lcd_touch_cst816s_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y,
    ...
    ESP_RETURN_ON_FALSE(point_num != NULL, false, TAG,
                        "Pointer to number of touch points can't be NULL");
...
static esp_err_t touch_cst816s_i2c_read(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data,
    ...
    ESP_RETURN_ON_FALSE(data != NULL, ESP_ERR_INVALID_ARG, TAG,
                        "Pointer to the data array can't be NULL");
