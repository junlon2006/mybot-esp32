 ...
 *
 * @note Local integration: waits up to five seconds for the task to finish
 * LVGL cleanup. The caller must first quiesce display/input callbacks and
 * must not hold the LVGL lock. Serialize init/deinit calls. On failure, port
 * resources remain owned by the port; retry deinit before another init.
 *
 * @return
 *      - ESP_OK                    on success
