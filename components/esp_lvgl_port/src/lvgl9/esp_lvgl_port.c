/*
 * SPDX-FileCopyrightText: 2024-2026 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */
/* Local modifications: joined lifecycle and generation-guarded timer cleanup;
 * see ../../PATCHES.md. */

#include <string.h>
#include "esp_system.h"
#include "esp_log.h"
#include "esp_err.h"
#include "esp_check.h"
#include "esp_timer.h"
#include "freertos/FreeRTOS.h"
#include "freertos/portmacro.h"
#include "freertos/task.h"
#include "freertos/semphr.h"
#include "freertos/event_groups.h"
#include "freertos/idf_additions.h"
#include "esp_lvgl_port.h"
#include "esp_lvgl_port_priv.h"
#include "lvgl.h"

static const char *TAG = "LVGL";

/*******************************************************************************
* Types definitions
*******************************************************************************/

typedef struct lvgl_port_ctx_s {
    TaskHandle_t        lvgl_task;
    SemaphoreHandle_t   lvgl_mux;
    EventGroupHandle_t  lvgl_events;
    esp_timer_handle_t  tick_timer;
    bool                stop_requested;
    bool                init_ready;
    bool                task_exited;
    esp_err_t           init_result;
    int                 task_max_sleep_ms;
    int                 timer_period_ms;
} lvgl_port_ctx_t;

/*******************************************************************************
* Local variables
*******************************************************************************/
static lvgl_port_ctx_t lvgl_port_ctx;
static portMUX_TYPE tick_lock = portMUX_INITIALIZER_UNLOCKED;
static uintptr_t tick_generation;
static bool tick_enabled;
static uint32_t tick_period_ms;

/*******************************************************************************
* Function definitions
*******************************************************************************/
static void lvgl_port_task(void *arg);
static esp_err_t lvgl_port_tick_init(void);
static void lvgl_port_disable_tick(void);
static inline bool indev_matches_event(const lv_indev_t *indev, const EventBits_t event);

/*******************************************************************************
* Public API functions
*******************************************************************************/

esp_err_t lvgl_port_init(const lvgl_port_cfg_t *cfg)
{
    esp_err_t ret = ESP_OK;
    ESP_RETURN_ON_FALSE(cfg && cfg->task_stack > 0 && cfg->timer_period_ms > 0 &&
                        cfg->task_priority >= 0 && cfg->task_priority < configMAX_PRIORITIES &&
                        cfg->task_max_sleep_ms >= 0,
                        ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    ESP_RETURN_ON_FALSE(cfg->task_affinity >= -1 && cfg->task_affinity < configNUM_CORES,
                        ESP_ERR_INVALID_ARG, TAG,
                        "Bad core number for task! Maximum core number is %d", configNUM_CORES - 1);
    ESP_RETURN_ON_FALSE(!lvgl_port_ctx.lvgl_task && !lvgl_port_ctx.lvgl_mux &&
                        !lvgl_port_ctx.lvgl_events && !lvgl_port_ctx.tick_timer,
                        ESP_ERR_INVALID_STATE, TAG, "LVGL port is initialized or cleanup is pending");

    memset(&lvgl_port_ctx, 0, sizeof(lvgl_port_ctx));

    /* Tick init */
    lvgl_port_ctx.timer_period_ms = cfg->timer_period_ms;
    /* Create task */
    lvgl_port_ctx.task_max_sleep_ms = cfg->task_max_sleep_ms;
    if (lvgl_port_ctx.task_max_sleep_ms == 0) {
        lvgl_port_ctx.task_max_sleep_ms = 500;
    }
    /* LVGL semaphore */
    lvgl_port_ctx.lvgl_mux = xSemaphoreCreateRecursiveMutex();
    ESP_GOTO_ON_FALSE(lvgl_port_ctx.lvgl_mux, ESP_ERR_NO_MEM, err, TAG, "Create LVGL mutex fail!");
    /* Task queue */
    lvgl_port_ctx.lvgl_events = xEventGroupCreate();
    ESP_GOTO_ON_FALSE(lvgl_port_ctx.lvgl_events, ESP_ERR_NO_MEM, err, TAG, "Create LVGL Event Group fail!");

    BaseType_t res;
    const uint32_t caps = cfg->task_stack_caps ? cfg->task_stack_caps : MALLOC_CAP_INTERNAL |
                          MALLOC_CAP_DEFAULT; // caps cannot be zero
    if (cfg->task_affinity < 0) {
        res = xTaskCreateWithCaps(lvgl_port_task, "taskLVGL", cfg->task_stack, NULL, cfg->task_priority,
                                  &lvgl_port_ctx.lvgl_task, caps);
    } else {
        res = xTaskCreatePinnedToCoreWithCaps(lvgl_port_task, "taskLVGL", cfg->task_stack, NULL,
                                              cfg->task_priority, &lvgl_port_ctx.lvgl_task, cfg->task_affinity, caps);
    }
    ESP_GOTO_ON_FALSE(res == pdPASS, ESP_FAIL, err, TAG, "Create LVGL task fail!");

    /* The worker publishes readiness only after both LVGL and its tick timer
     * are ready. No notification can target an init caller that timed out. */
    int64_t deadline = esp_timer_get_time() + 5000000;
    while (!__atomic_load_n(&lvgl_port_ctx.init_ready, __ATOMIC_ACQUIRE)) {
        if (esp_timer_get_time() >= deadline) {
            ret = ESP_ERR_TIMEOUT;
            goto err;
        }
        vTaskDelay(1);
    }
    ret = lvgl_port_ctx.init_result;

err:
    if (ret != ESP_OK) {
        esp_err_t cleanup = lvgl_port_deinit();
        if (cleanup != ESP_OK) {
            ESP_LOGE(TAG, "Initialization cleanup pending: %s", esp_err_to_name(cleanup));
        }
    }

    return ret;
}

esp_err_t lvgl_port_resume(void)
{
    esp_err_t ret = ESP_ERR_INVALID_STATE;

    if (lvgl_port_ctx.tick_timer != NULL) {
        lv_timer_enable(true);
        ret = esp_timer_start_periodic(lvgl_port_ctx.tick_timer, (uint64_t)lvgl_port_ctx.timer_period_ms * 1000);
    }

    return ret;
}

esp_err_t lvgl_port_stop(void)
{
    esp_err_t ret = ESP_ERR_INVALID_STATE;

    if (lvgl_port_ctx.tick_timer != NULL) {
        lv_timer_enable(false);
        ret = esp_timer_stop(lvgl_port_ctx.tick_timer);
    }

    return ret;
}

esp_err_t lvgl_port_deinit(void)
{
    ESP_RETURN_ON_FALSE(!lvgl_port_ctx.lvgl_task ||
                        xTaskGetCurrentTaskHandle() != lvgl_port_ctx.lvgl_task,
                        ESP_ERR_INVALID_STATE, TAG, "Cannot join the LVGL task from itself");

    __atomic_store_n(&lvgl_port_ctx.stop_requested, true, __ATOMIC_RELEASE);
    if (lvgl_port_ctx.lvgl_events) {
        xEventGroupSetBits(lvgl_port_ctx.lvgl_events, LVGL_PORT_EVENT_USER);
    }
    if (lvgl_port_ctx.lvgl_task) {
        int64_t deadline = esp_timer_get_time() + 5000000;
        while (!__atomic_load_n(&lvgl_port_ctx.task_exited, __ATOMIC_ACQUIRE)) {
            if (esp_timer_get_time() >= deadline) {
                return ESP_ERR_TIMEOUT;
            }
            vTaskDelay(1);
        }
    }

    /* The worker has stopped accessing LVGL and all port objects. Failed init
     * before task creation reaches the same synchronous cleanup below. */
    lvgl_port_disable_tick();
    if (lvgl_port_ctx.tick_timer) {
        esp_err_t err = esp_timer_stop(lvgl_port_ctx.tick_timer);
        if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
            return err;
        }
        err = esp_timer_delete(lvgl_port_ctx.tick_timer);
        if (err != ESP_OK) {
            return err;
        }
        lvgl_port_ctx.tick_timer = NULL;
    }
    if (lvgl_port_ctx.lvgl_mux) {
        vSemaphoreDelete(lvgl_port_ctx.lvgl_mux);
    }
    if (lvgl_port_ctx.lvgl_events) {
        vEventGroupDelete(lvgl_port_ctx.lvgl_events);
    }
    memset(&lvgl_port_ctx, 0, sizeof(lvgl_port_ctx));

    return ESP_OK;
}

bool lvgl_port_lock(uint32_t timeout_ms)
{
    assert(lvgl_port_ctx.lvgl_mux && "lvgl_port_init must be called first");

    const TickType_t timeout_ticks = (timeout_ms == 0) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
    return xSemaphoreTakeRecursive(lvgl_port_ctx.lvgl_mux, timeout_ticks) == pdTRUE;
}

void lvgl_port_unlock(void)
{
    assert(lvgl_port_ctx.lvgl_mux && "lvgl_port_init must be called first");
    xSemaphoreGiveRecursive(lvgl_port_ctx.lvgl_mux);
}

esp_err_t lvgl_port_task_wake(lvgl_port_event_type_t event, void *param)
{
    EventBits_t bits = 0;
    if (!lvgl_port_ctx.lvgl_events) {
        return ESP_ERR_INVALID_STATE;
    }

    /* Get unprocessed bits */
    if (xPortInIsrContext() == pdTRUE) {
        bits = xEventGroupGetBitsFromISR(lvgl_port_ctx.lvgl_events);
    } else {
        bits = xEventGroupGetBits(lvgl_port_ctx.lvgl_events);
    }

    /* Set event */
    bits |= event;

    /* Save */
    if (xPortInIsrContext() == pdTRUE) {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        xEventGroupSetBitsFromISR(lvgl_port_ctx.lvgl_events, bits, &xHigherPriorityTaskWoken);
        if (xHigherPriorityTaskWoken) {
            portYIELD_FROM_ISR( );
        }
    } else {
        xEventGroupSetBits(lvgl_port_ctx.lvgl_events, bits);
    }

    return ESP_OK;
}

IRAM_ATTR bool lvgl_port_task_notify(uint32_t value)
{
    BaseType_t need_yield = pdFALSE;
    if (!lvgl_port_ctx.lvgl_task ||
        __atomic_load_n(&lvgl_port_ctx.task_exited, __ATOMIC_ACQUIRE)) {
        return false;
    }

    // Notify LVGL task
    if (xPortInIsrContext() == pdTRUE) {
        xTaskNotifyFromISR(lvgl_port_ctx.lvgl_task, value, eNoAction, &need_yield);
    } else {
        xTaskNotify(lvgl_port_ctx.lvgl_task, value, eNoAction);
    }

    return (need_yield == pdTRUE);
}

/*******************************************************************************
* Private functions
*******************************************************************************/

static void lvgl_port_task(void *arg)
{
    (void)arg;
    EventBits_t events = 0;
    uint32_t task_delay_ms = 0;
    lv_indev_t *indev = NULL;

    /* LVGL init */
    lv_init();
    /* Tick init */
    lvgl_port_ctx.init_result = lvgl_port_tick_init();
    __atomic_store_n(&lvgl_port_ctx.init_ready, true, __ATOMIC_RELEASE);

    if (lvgl_port_ctx.init_result == ESP_OK) {
        ESP_LOGI(TAG, "Starting LVGL task");
    }
    while (lvgl_port_ctx.init_result == ESP_OK &&
           !__atomic_load_n(&lvgl_port_ctx.stop_requested, __ATOMIC_ACQUIRE)) {
        /* Wait for queue or timeout (sleep task) */
        TickType_t wait = (pdMS_TO_TICKS(task_delay_ms) >= 1 ? pdMS_TO_TICKS(task_delay_ms) : 1);
        events = xEventGroupWaitBits(lvgl_port_ctx.lvgl_events, 0xFF, pdTRUE, pdFALSE, wait);
        if (__atomic_load_n(&lvgl_port_ctx.stop_requested, __ATOMIC_ACQUIRE)) {
            break;
        }

        if (lvgl_port_lock(0)) {
            if (!lv_display_get_default()) {
                lvgl_port_unlock();
                task_delay_ms = 1;
                vTaskDelay(1);
                continue;
            }

            /* Call read input devices */
            if (events & LVGL_PORT_EVENT_TOUCH || events & LVGL_PORT_EVENT_ENCODER) {
                indev = lv_indev_get_next(NULL);
                while (indev != NULL) {
                    if (indev_matches_event(indev, events)) {
                        lv_indev_read(indev);
                    }
                    indev = lv_indev_get_next(indev);
                }
            }

            /* Handle LVGL */
            task_delay_ms = lv_timer_handler();
            lvgl_port_unlock();
        } else {
            task_delay_ms = 1; /*Keep trying*/
        }

        if (task_delay_ms == LV_NO_TIMER_READY) {
            task_delay_ms = lvgl_port_ctx.task_max_sleep_ms;
        }

        /* Minimal dealy for the task. When there is too much events, it takes time for other tasks and interrupts. */
        vTaskDelay(1);
    }

    ESP_LOGI(TAG, "Stopped LVGL task");

    /* Disable any current or already dispatched tick before deleting LVGL.
     * A late callback sees the static generation guard and touches no context. */
    lvgl_port_disable_tick();
    lv_deinit();

    /* Final port-context access. The caller can now release semaphores and
     * reinitialize; FreeRTOS owns this task's separate stack and TCB. */
    __atomic_store_n(&lvgl_port_ctx.task_exited, true, __ATOMIC_RELEASE);
    vTaskDeleteWithCaps( NULL );
}

static void lvgl_port_disable_tick(void)
{
    portENTER_CRITICAL(&tick_lock);
    tick_enabled = false;
    portEXIT_CRITICAL(&tick_lock);
}

static void lvgl_port_tick_increment(void *arg)
{
    portENTER_CRITICAL(&tick_lock);
    if (tick_enabled && (uintptr_t)arg == tick_generation) {
        lv_tick_inc(tick_period_ms);
    }
    portEXIT_CRITICAL(&tick_lock);
}

static esp_err_t lvgl_port_tick_init(void)
{
    portENTER_CRITICAL(&tick_lock);
    ++tick_generation;
    if (tick_generation == 0) {
        ++tick_generation;
    }
    uintptr_t generation = tick_generation;
    tick_period_ms = lvgl_port_ctx.timer_period_ms;
    tick_enabled = false;
    portEXIT_CRITICAL(&tick_lock);

    const esp_timer_create_args_t lvgl_tick_timer_args = {
        .callback = &lvgl_port_tick_increment,
        .arg = (void *)generation,
        .name = "LVGL tick",
    };
    ESP_RETURN_ON_ERROR(esp_timer_create(&lvgl_tick_timer_args, &lvgl_port_ctx.tick_timer), TAG,
                        "Creating LVGL timer filed!");
    esp_err_t ret = esp_timer_start_periodic(lvgl_port_ctx.tick_timer,
                                            (uint64_t)lvgl_port_ctx.timer_period_ms * 1000);
    if (ret == ESP_OK) {
        portENTER_CRITICAL(&tick_lock);
        tick_enabled = true;
        portEXIT_CRITICAL(&tick_lock);
    }
    return ret;
}

static inline bool indev_matches_event(const lv_indev_t *indev, const EventBits_t event)
{
    assert(indev != NULL);

    bool does_match = false;

    switch (lv_indev_get_type(indev)) {
    case LV_INDEV_TYPE_KEYPAD:
    case LV_INDEV_TYPE_POINTER:
        does_match = event & LVGL_PORT_EVENT_TOUCH;
        break;
    case LV_INDEV_TYPE_ENCODER:
        does_match = event & LVGL_PORT_EVENT_ENCODER;
        break;
    default:
        does_match = false;
    }
    return does_match;
}
