 ...
 */
/* Local modifications: allocation rollback and failed-flush completion;
 * see ../../PATCHES.md. */
...
{
    ESP_RETURN_ON_FALSE(disp_cfg && disp_cfg->io_handle && disp_cfg->panel_handle &&
                        disp_cfg->buffer_size > 0 && disp_cfg->hres > 0 && disp_cfg->vres > 0,
                        NULL, TAG, "Invalid display configuration");
    lvgl_port_lock(0);
    ...
    if (disp != NULL) {
        ...
        esp_err_t err = esp_lcd_panel_io_register_event_callbacks(disp_ctx->io_handle, &cbs, disp);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Register display callback failed: %s", esp_err_to_name(err));
        ...
        const lvgl_port_disp_priv_cfg_t *priv_cfg)
{
    ...
    * remains accessible when the cache is disabled during SPI flash operations. */
    disp_ctx = heap_caps_malloc(sizeof(lvgl_port_display_ctx_t),
                                        MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
#else
    disp_ctx = malloc(sizeof(lvgl_port_display_ctx_t));
#endif
    ...
    ESP_GOTO_ON_FALSE(disp, ESP_ERR_NO_MEM, err, TAG, "Not enough memory for LVGL display!");
...
#error please disable LV_COLOR_DEPTH_1 for using monochromatic screen
        ...
        ESP_GOTO_ON_FALSE(display_color_format == LV_COLOR_FORMAT_RGB565
...
static void lvgl_port_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map)
{
    ...
    if ((disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_RGB || disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_DSI)
        ...
        if (lv_disp_flush_is_last(drv)) {
            ...
            esp_err_t err = esp_lcd_panel_draw_bitmap(disp_ctx->panel_handle, 0, 0,
                                                      lv_disp_get_hor_res(drv), lv_disp_get_ver_res(drv), color_map);
            if (err != ESP_OK) {
        ...
        esp_err_t err = esp_lcd_panel_draw_bitmap(disp_ctx->panel_handle, offsetx1, offsety1,
