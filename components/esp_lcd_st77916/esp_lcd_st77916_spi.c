 ...
 */
/* Modified for local vendoring: formatting normalized for this repository. */
...
static esp_err_t panel_st77916_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start,
...
esp_err_t esp_lcd_new_panel_st77916_spi(const esp_lcd_panel_io_handle_t io,
    ...
    case 18: // RGB666
        ...
        // each color component (R/G/B) should occupy the 6 high bits of a byte, which means 3 full
        // bytes are required for a pixel
    ...
    st77916_vendor_config_t *vendor_config =
        (st77916_vendor_config_t *)panel_dev_config->vendor_config;
    if (vendor_config) {
    ...
    *ret_panel = &(st77916->base);
    ...
    ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d", ESP_LCD_ST77916_VER_MAJOR,
             ESP_LCD_ST77916_VER_MINOR, ESP_LCD_ST77916_VER_PATCH);
...
static esp_err_t tx_param(st77916_panel_t *st77916, esp_lcd_panel_io_handle_t io, int lcd_cmd,
...
static esp_err_t tx_color(st77916_panel_t *st77916, esp_lcd_panel_io_handle_t io, int lcd_cmd,
                          const void *param, size_t param_size) {
    if (st77916->flags.use_qspi_interface) {
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
...
static esp_err_t panel_st77916_reset(esp_lcd_panel_t *panel) {
    ...
    if (st77916->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_SWRESET, NULL, 0), TAG,
                            "send command failed");
        vTaskDelay(pdMS_TO_TICKS(120));
    }
...
static const st77916_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0xE0,
     (uint8_t[]){0x70, 0x09, 0x12, 0x0C, 0x0B, 0x27, 0x38, 0x54, 0x4E, 0x19, 0x15, 0x15, 0x2C,
                 0x2F},
     14, 0},
    {0xE1,
     (uint8_t[]){0x70, 0x08, 0x11, 0x0C, 0x0B, 0x27, 0x38, 0x43, 0x4C, 0x18, 0x14, 0x14, 0x2B,
                 0x2D},
     14, 0},
...
static esp_err_t panel_st77916_init(esp_lcd_panel_t *panel) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_MADCTL,
                                 (uint8_t[]){
                                     st77916->madctl_val,
                                 },
                                 1),
                        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_COLMOD,
                                 (uint8_t[]){
                                     st77916->colmod_val,
                                 },
                                 1),
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        // Send command
        ESP_RETURN_ON_ERROR(
            tx_param(st77916, io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes),
            TAG, "send command failed");
...
static esp_err_t panel_st77916_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start,
    ...
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_CASET,
                                 (uint8_t[]){
                                     (x_start >> 8) & 0xFF,
                                     x_start & 0xFF,
                                     ((x_end - 1) >> 8) & 0xFF,
                                 ...
                                 4),
                        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_RASET,
                                 (uint8_t[]){
                                     (y_start >> 8) & 0xFF,
                                     y_start & 0xFF,
                                     ((y_end - 1) >> 8) & 0xFF,
    ...
    ESP_RETURN_ON_ERROR(tx_color(st77916, io, LCD_CMD_RAMWR, color_data, len), TAG,
...
static esp_err_t panel_st77916_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_MADCTL, (uint8_t[]){st77916->madctl_val}, 1),
                        TAG, "send command failed");
    return ret;
}
