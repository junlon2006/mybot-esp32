...
#include "display/vocat_renderer.h"
...
int mybot_display_panel_open(mybot_display_panel_t *panel) {
    ...
    const mybot_lcd_ops_t *ops = mybot_vocat_legacy_renderer_ops();
    if (!ops || ops->init(&s_legacy_context) < 0) {
        s_legacy_context = NULL;
        return -1;
    }
    panel->io = mybot_vocat_legacy_panel_io(s_legacy_context);
    panel->panel = mybot_vocat_legacy_panel(s_legacy_context);
...
int mybot_display_panel_close(mybot_display_panel_t *panel) {
    ...
    if (mybot_vocat_legacy_panel_io(s_legacy_context) ||
        mybot_vocat_legacy_panel(s_legacy_context)) {
        return -1;
    }
    s_legacy_context = NULL;
    panel->io = NULL;
