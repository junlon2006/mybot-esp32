...
/* Board-selected SPD2010 panel provider for the shared LVGL adapter. */
...
int mybot_display_panel_open(mybot_display_panel_t *panel) {
    return mybot_sensecap_lvgl_panel_open(panel, NULL, NULL);
}

int mybot_display_panel_close(mybot_display_panel_t *panel) {
    return mybot_sensecap_lvgl_panel_close(panel);
}
