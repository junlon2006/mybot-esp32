...
static uint16_t s_draw_buffer[LCD_WIDTH * LCD_TRANSFER_ROWS];
...
static int submit_bitmap(int x, int y, int width, int height) {
    ...
    return xSemaphoreTake(s_context.transfer_done, pdMS_TO_TICKS(LCD_TRANSFER_TIMEOUT_MS)) == pdTRUE
               ? 0
               : -1;
}
...
static int fill_screen(uint16_t color) {
    ...
    for (int y = 0; y < LCD_HEIGHT; y += LCD_TRANSFER_ROWS) {
        ...
        if (submit_bitmap(0, y, LCD_WIDTH, rows) < 0) {
            return -1;
        }
    }
    return 0;
...
static int draw_character(int x, int y, char character, int scale, uint16_t foreground,
    ...
    for (int pixel_y = 0; pixel_y < height; ++pixel_y) {
        int glyph_y = pixel_y / scale;
        for (int pixel_x = 0; pixel_x < width; ++pixel_x) {
            int glyph_x = pixel_x / scale;
            bool enabled = glyph_x < 5 && (glyph[glyph_x] & (1U << glyph_y)) != 0;
...
static int lcd_init(void **out_ctx) {
    ...
    *out_ctx = NULL;
    ...
    if (esp_lcd_new_panel_st7789(s_context.io, &panel_config, &s_context.panel) != ESP_OK ||
...
static int lcd_render(void *opaque, const mybot_lcd_content_t *content) {
    ...
    const char *label = "MYBOT";
    uint16_t background = RGB565(15, 20, 24);
    ...
    if (content->screen == MYBOT_LCD_SCREEN_PAIR_CODE) {
        size_t code_length = strlen(content->pair_code);
...
static void lcd_destroy(void *opaque) {
    ...
    spi_bus_free(SPI3_HOST);
    vSemaphoreDelete(ctx->transfer_done);
    *ctx = (lcd_context_t){0};
}
