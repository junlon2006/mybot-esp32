...
static int release_transport(void) {
    ...
    if (mybot_sticks3_set_display_backlight(0) < 0) {
        ESP_LOGW(TAG, "event=lcd action=backlight_off result=error");
    }
    s_context.backlight_enabled = false;
    if (s_context.panel) {
        if (!s_context.transfer_failed &&
    ...
    if (s_context.spi_ready) {
        esp_err_t result = spi_bus_free(MYBOT_STICKS3_LCD_SPI_HOST);
        if (result != ESP_OK) {
...
static int initialize_transport(void) {
    ...
    if (mybot_sticks3_set_display_backlight(0) < 0) {
        ESP_LOGE(TAG, "event=lcd action=initialize component=backlight result=error");
        return -1;
    }
    ...
    esp_err_t result = spi_bus_initialize(MYBOT_STICKS3_LCD_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO);
    if (result != ESP_OK) {
    ...
    const esp_lcd_panel_io_spi_config_t io_config = {
        ...
        .spi_mode = MYBOT_STICKS3_LCD_SPI_MODE,
        .pclk_hz = MYBOT_STICKS3_LCD_PIXEL_CLOCK_HZ,
        .trans_queue_depth = 2,
    ...
    result = esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)MYBOT_STICKS3_LCD_SPI_HOST,
    ...
    result = esp_lcd_new_panel_st7789(s_context.io, &panel_config, &s_context.panel);
    if (result == ESP_OK) {
    ...
    if (result != ESP_OK) {
        s_context.transfer_failed = true;
        ESP_LOGE(TAG, "event=lcd action=initialize component=st7789p3 result=error code=%s",
                 esp_err_to_name(result));
...
static int lcd_init(void **out_context) {
    ...
    *out_context = &s_context;
    ESP_LOGI(TAG,
             "event=lcd action=initialize result=ok controller=st7789p3 width=%d height=%d "
...
static int lcd_render(void *opaque, const mybot_lcd_content_t *content) {
    ...
    text_layer_t layers[2] = {
        {
            .text = label,
            .y = content->screen == MYBOT_LCD_SCREEN_PAIR_CODE ? 66 : 105,
            .preferred_scale = 3,
            .color = foreground,
        },
    };
    ...
    if (!context->backlight_enabled) {
        if (mybot_sticks3_set_display_backlight(LCD_BACKLIGHT_PERCENT) < 0) {
            ESP_LOGE(TAG, "event=lcd action=backlight_on result=error");
            return -1;
        }
        context->backlight_enabled = true;
    }
    return 0;
}
