...
static int set_speaker_power_locked(bool enabled) {
...
static int close_codec_locked(void) {
    if (!s_audio.device || !s_audio.codec_open) {
        return 0;
    }

    int result = esp_codec_dev_close(s_audio.device);
...
static bool shared_has_resources(void) {
    return s_audio.tx_channel || s_audio.rx_channel || s_audio.ctrl_if || s_audio.codec_if ||
           s_audio.gpio_if || s_audio.device || s_audio.speaker_enabled;
}
...
static int shared_release_locked(void) {
    ...
    if (s_audio.codec_if) {
        if (audio_codec_delete_codec_if(s_audio.codec_if) != ESP_CODEC_DEV_OK) {
    ...
    if (s_audio.ctrl_if) {
        if (audio_codec_delete_ctrl_if(s_audio.ctrl_if) != ESP_CODEC_DEV_OK) {
...
static int shared_acquire_locked(audio_direction_t direction) {
    ...
    if (!s_audio.ready) {
        ...
        if (initialize_i2s_locked() < 0 || initialize_codec_locked() < 0) {
...
static int open_codec_locked(void) {
    ...
    if (esp_codec_dev_open(s_audio.device, &format) != ESP_CODEC_DEV_OK) {
        ESP_LOGE(TAG, "event=audio_device component=es8311 action=open result=error");
        (void)esp_codec_dev_close(s_audio.device);
        return -1;
    }
...
static int playback_start(void *opaque) {
    ...
    int hardware_volume = hardware_volume_from_logical(s_output_volume);
    if (esp_codec_dev_set_out_vol(s_audio.device, hardware_volume) != ESP_CODEC_DEV_OK) {
    ...
    if (set_speaker_power_locked(true) < 0 ||
        mybot_pcm_playback_buffer_start(context->playback_buffer) < 0) {
        (void)set_speaker_power_locked(false);
        if (!s_audio.capture_started) {
            (void)close_codec_locked();
            (void)disable_channel(s_audio.tx_channel, &s_audio.tx_enabled, "playback");
        }
...
static int stream_stop(void *opaque) {
    ...
    if (context->direction == AUDIO_DIRECTION_CAPTURE) {
        ...
        if (!s_audio.playback_started && close_codec_locked() < 0) {
            result = -1;
        }
        if (!s_audio.playback_started &&
        ...
        if (set_speaker_power_locked(false) < 0 ||
            (!s_audio.capture_started && close_codec_locked() < 0)) {
            audio_unlock();
            return -1;
        }
...
static int volume_set(void *opaque, int volume) {
    ...
    int hardware_volume = hardware_volume_from_logical(volume);
    int previous_volume = s_output_volume;
    if (s_audio.playback_started &&
        esp_codec_dev_set_out_vol(s_audio.device, hardware_volume) != ESP_CODEC_DEV_OK) {
        audio_unlock();
        ESP_LOGE(TAG,
                 "event=volume action=set value=%d hardware_value=%d result=error reason=codec",
                 volume, hardware_volume);
        return -1;
    }
    s_output_volume = volume;
    ...
    if (persist_volume(context, volume) < 0) {
        ...
        audio_unlock();
        ESP_LOGW(TAG,
                 "event=volume action=set value=%d hardware_value=%d applied=1 persisted=0 "
    ...
    audio_unlock();
    ESP_LOGI(TAG, "event=volume action=set value=%d hardware_value=%d persisted=1 result=ok",
             volume, hardware_volume);
    return 0;
}
