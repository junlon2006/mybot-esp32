...
static vocat_audio_shared_t s_audio;
...
static int initialize_i2s_locked(void) {
    gpio_num_t din_gpio = mybot_vocat_audio_din_gpio();
    if (din_gpio == GPIO_NUM_NC) {
        ESP_LOGE(TAG, "event=audio_device action=initialize result=error reason=audio_din");
        return -1;
    }
...
static int initialize_codecs_locked(void) {
    i2c_master_bus_handle_t i2c_bus = mybot_vocat_i2c_bus_handle();
    if (!i2c_bus) {
        ESP_LOGE(TAG, "event=audio_device action=initialize result=error reason=i2c_bus");
        return -1;
    }
    gpio_num_t pa_gpio = mybot_vocat_audio_pa_gpio();
    if (pa_gpio == GPIO_NUM_NC) {
        ESP_LOGE(TAG, "event=audio_device action=initialize result=error reason=speaker_pa");
        return -1;
    }
    ...
    audio_codec_i2c_cfg_t i2c_config = {
        .port = MYBOT_VOCAT_I2C_PORT,
        .addr = MYBOT_VOCAT_ES8311_ADDRESS,
        .bus_handle = i2c_bus,
    };
    ...
    i2c_config.addr = MYBOT_VOCAT_ES7210_ADDRESS;
...
static esp_err_t playback_sink_write(void *opaque, const int16_t *pcm, size_t frames,
    ...
    *written_frames = 0;
    ...
    for (size_t frame = 0; frame < frames; ++frame) {
        context->scratch[frame * AUDIO_TX_CHANNELS] = pcm[frame];
        context->scratch[frame * AUDIO_TX_CHANNELS + 1] = pcm[frame];
    }
    const size_t frame_bytes = AUDIO_TX_CHANNELS * sizeof(context->scratch[0]);
...
static int stream_init(void **out_context, int rate, int channels, int bits,
    ...
    *out_context = NULL;
    ...
    size_t wire_channels =
        direction == AUDIO_DIRECTION_CAPTURE ? AUDIO_RX_CHANNELS : AUDIO_TX_CHANNELS;
    context->scratch = calloc(AUDIO_MAX_FRAMES * wire_channels, sizeof(*context->scratch));
    if (!context->scratch) {
        free(context);
        return -1;
    }
...
static int open_input_codec_locked(void) {
    ...
    esp_codec_dev_sample_info_t format = {
        .bits_per_sample = 16,
        .channel = AUDIO_RX_CHANNELS,
        .channel_mask = ESP_CODEC_DEV_MAKE_CHANNEL_MASK(AUDIO_CAPTURE_SLOT),
    ...
    if (esp_codec_dev_set_in_gain(s_audio.input_device, MYBOT_VOCAT_AUDIO_INPUT_GAIN_DB) !=
        ESP_CODEC_DEV_OK) {
...
static int capture_read(void *opaque, void *buffer, int frames) {
    ...
    esp_err_t err =
        i2s_channel_read(s_audio.rx_channel, context->scratch,
                         (size_t)frames * AUDIO_RX_CHANNELS * sizeof(context->scratch[0]),
                         &bytes_read, AUDIO_IO_TIMEOUT_MS);
    if (err != ESP_OK && err != ESP_ERR_TIMEOUT) {
        return aosl_atomic_read(&context->started) ? -1 : 0;
    }

    int captured_frames = (int)(bytes_read / (AUDIO_RX_CHANNELS * sizeof(context->scratch[0])));
    int16_t *output = buffer;
    for (int frame = 0; frame < captured_frames; ++frame) {
        output[frame] = context->scratch[frame * AUDIO_RX_CHANNELS + AUDIO_CAPTURE_SLOT];
    }
...
const mybot_audio_capture_ops_t *mybot_vocat_audio_capture_ops(void) {
    return &s_capture_ops;
}

const mybot_audio_playback_ops_t *mybot_vocat_audio_playback_ops(void) {
    return &s_playback_ops;
}

const mybot_audio_volume_ops_t *mybot_vocat_audio_volume_ops(void) {
    return &s_volume_ops;
}
