...
static int playback_write(void *opaque, const void *buffer, int frames) {
    ...
    for (int frame = 0; frame < frames; ++frame) {
        context->scratch[frame * AUDIO_TX_CHANNELS] = input[frame];
        context->scratch[frame * AUDIO_TX_CHANNELS + 1] = input[frame];
    }

    size_t bytes_written = 0;
    esp_err_t err =
        i2s_channel_write(s_audio.tx_channel, context->scratch,
                          (size_t)frames * AUDIO_TX_CHANNELS * sizeof(context->scratch[0]),
                          &bytes_written, AUDIO_IO_TIMEOUT_MS);
    if (err != ESP_OK && err != ESP_ERR_TIMEOUT) {
        return aosl_atomic_read(&context->started) ? -1 : 0;
    }
    return (int)(bytes_written / (AUDIO_TX_CHANNELS * sizeof(context->scratch[0])));
}
