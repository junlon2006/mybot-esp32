...
static sensecap_audio_shared_t s_audio;
...
static bool shared_has_codec_resources(void) {
    return s_audio.output_device || s_audio.input_device || s_audio.output_codec_if ||
           s_audio.input_codec_if || s_audio.output_ctrl_if || s_audio.input_ctrl_if ||
           s_audio.gpio_if;
}
...
static int shared_release_locked(void) {
    ...
    if (s_audio.input_codec_if) {
        if (audio_codec_delete_codec_if(s_audio.input_codec_if) != ESP_CODEC_DEV_OK) {
            ESP_LOGW(TAG, "event=audio_device component=es7243e action=destroy result=error");
            result = -1;
        }
    ...
    if (s_audio.input_ctrl_if) {
        if (audio_codec_delete_ctrl_if(s_audio.input_ctrl_if) != ESP_CODEC_DEV_OK) {
            ESP_LOGW(TAG, "event=audio_device component=es7243e_i2c action=destroy result=error");
            result = -1;
        }
    ...
    if (!s_audio.tx_channel && !s_audio.rx_channel && !shared_has_codec_resources() &&
        s_audio.power_enabled) {
        if (mybot_sensecap_set_codec_power(false) != 0) {
            ESP_LOGW(TAG, "event=audio_device component=codec_power action=disable result=error");
            result = -1;
        } else {
            s_audio.power_enabled = false;
        }
    }
    return result;
}

static int enable_codec_power_locked(void) {
    if (s_audio.power_enabled) {
        return 0;
    }
    if (mybot_sensecap_set_codec_power(true) != 0) {
        ESP_LOGE(TAG, "event=audio_device component=codec_power action=enable result=error");
        return -1;
    }
    s_audio.power_enabled = true;
    return 0;
}
...
static int initialize_codecs_locked(void) {
    void *i2c_bus = mybot_sensecap_i2c_bus_handle();
    if (!i2c_bus) {
    ...
    audio_codec_i2c_cfg_t i2c_config = {
        .port = MYBOT_SENSECAP_I2C_PORT,
        .addr = MYBOT_SENSECAP_ES8311_ADDRESS,
        .bus_handle = i2c_bus,
    };
    ...
    i2c_config.addr = MYBOT_SENSECAP_ES7243E_ADDRESS;
    s_audio.input_ctrl_if = audio_codec_new_i2c_ctrl(&i2c_config);
    if (!s_audio.input_ctrl_if) {
        goto codec_error;
    }

    es7243e_codec_cfg_t input_codec_config = {
        .ctrl_if = s_audio.input_ctrl_if,
    };
    s_audio.input_codec_if = es7243e_codec_new(&input_codec_config);
    if (!s_audio.input_codec_if) {
        goto codec_error;
    }
    if (!s_audio.input_codec_if->enable ||
        s_audio.input_codec_if->enable(s_audio.input_codec_if, false) != ESP_CODEC_DEV_OK) {
        goto codec_error;
    }
    ...
    if (!s_audio.ready) {
        if (s_audio.tx_channel || s_audio.rx_channel || shared_has_codec_resources()) {
            if (shared_release_locked() < 0) {
                ESP_LOGE(TAG, "event=audio_device action=initialize result=error "
                              "reason=previous_cleanup");
                return -1;
            }
        }
        if (enable_codec_power_locked() < 0 || initialize_i2s_locked() < 0 ||
            initialize_codecs_locked() < 0) {
        ...
        ESP_LOGI(TAG,
                 ...
                 "capture_channel=right duplex=1 result=ok",
...
/* Only the playback worker owns this stream's conversion scratch buffer. */
static esp_err_t playback_sink_write(void *opaque, const int16_t *pcm, size_t frames,
    ...
    *written_frames = 0;
    if (!context || !pcm || frames == 0 || frames > AUDIO_MAX_FRAMES) {
        return ESP_ERR_INVALID_ARG;
    }
    for (size_t frame = 0; frame < frames; ++frame) {
        context->scratch[frame * AUDIO_WIRE_CHANNELS] = pcm[frame];
        context->scratch[frame * AUDIO_WIRE_CHANNELS + 1] = pcm[frame];
    }
    const size_t frame_bytes = AUDIO_WIRE_CHANNELS * sizeof(context->scratch[0]);
    size_t bytes_written = 0;
    esp_err_t err = i2s_channel_write(s_audio.tx_channel, context->scratch, frames * frame_bytes,
                                      &bytes_written, timeout_ms);
    if (bytes_written > frames * frame_bytes || bytes_written % frame_bytes != 0) {
        return ESP_FAIL;
    }
...
static int capture_start(void *opaque) {
    ...
    int result = -1;
    if (!s_audio.ready || context->stop_pending || !s_audio.capture_attached ||
        s_audio.capture_started || enable_tx_locked() < 0) {
        goto done;
    }
    ...
    if (esp_codec_dev_open(s_audio.input_device, &format) != ESP_CODEC_DEV_OK ||
        esp_codec_dev_set_in_gain(s_audio.input_device, MYBOT_SENSECAP_INPUT_GAIN_DB) !=
            ESP_CODEC_DEV_OK) {
        ESP_LOGE(TAG,
                 "event=audio_device direction=capture action=start result=error reason=codec");
...
static int playback_start(void *opaque) {
    ...
    int result = -1;
    if (!s_audio.ready || context->stop_pending || !s_audio.playback_attached ||
        s_audio.playback_started || enable_tx_locked() < 0) {
        goto done;
    }
    ...
    if (esp_codec_dev_open(s_audio.output_device, &format) != ESP_CODEC_DEV_OK ||
        ...
        mybot_pcm_playback_buffer_start(context->playback_buffer) < 0) {
        ESP_LOGE(TAG,
                 "event=audio_device direction=playback action=start result=error reason=codec");
...
static int stream_stop(void *opaque) {
    ...
    if (context->direction == AUDIO_DIRECTION_CAPTURE) {
        ...
        /* Drain old DMA contents while muted. Keep TX clocking when capture still owns it. */
        if (s_audio.tx_enabled) {
            vTaskDelay(pdMS_TO_TICKS(AUDIO_DMA_CLEAR_MS));
        }
        if (!s_audio.capture_started &&
            disable_channel(s_audio.tx_channel, &s_audio.tx_enabled, "playback") < 0) {
            result = -1;
