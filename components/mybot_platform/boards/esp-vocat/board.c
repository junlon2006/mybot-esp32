...
static void board_on_provisioning(const char *trigger) {
    ...
    (void)mybot_embedded_ogg_play_wifi_provisioning(mybot_vocat_audio_playback_ops(),
                                                    mybot_vocat_audio_volume_ops(), trigger);
}
...
static void board_rollback_prepare(void) {
    int result = mybot_vocat_input_stop();
    if (s_lcd_ops && s_lcd_context) {
...
static int board_prepare(void) {
    if (mybot_vocat_hardware_init() < 0) {
...
static int board_register_platform(void) {
    const mybot_platform_descriptor_t descriptor = {
        ...
        .key = mybot_vocat_input_ops(),
        .audio_capture = mybot_vocat_audio_capture_ops(),
        .audio_playback = mybot_vocat_audio_playback_ops(),
        .audio_volume = mybot_vocat_audio_volume_ops(),
        .announce = mybot_esp32s3_announce_ops(),
        .https = mybot_esp32s3_https_ops(),
        .lcd = mybot_vocat_lcd_ops(),
    };
