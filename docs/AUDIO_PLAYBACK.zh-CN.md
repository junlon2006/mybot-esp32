...
16 kHz、单声道 signed-16 PCM，独立任务持续向 I2S 供数。采集时钟、codec 初始化及
随附 SDK 均保持不变。

## 为什么增加缓冲

SDK 每 60 ms 提供 960 帧 PCM，此前各驱动直接将这些数据包写入 I2S，突发补数和 DMA
...
配网提示音及离线测试停止前使用单独的排空操作，等待 FIFO 和硬件尾音完成。最后一次
...
`CONFIG_MYBOT_DEBUG_RESOURCE_MONITOR` 还可开启周期播放耗时及错误统计，描述的是
独立任务的 I2S 写入，而非 SDK 数据包供数。`rebuffer_events` 估算重新缓冲的次数，
可能包含正常语音停顿，不是 DMA 断供计数。

公共实现基于已通过视频开启、关闭两种情况下真机测试的 CoreS3 缓冲方案提取。
提取后的实现及其他板型仍需真机回归，主机模拟及固件编译不能证明实际听音结果。
