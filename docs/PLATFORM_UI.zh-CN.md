...
AtomEchoS3R 与 ReSpeaker Flex 没有 LCD，不编入 LVGL。ML307、Wi-Fi 及音视频协议与显示后端
相互独立。
...
配对码页面提示用户在网页控制台输入配对码；提示超出底部标签宽度时才循环滚动，离开
配对码页面后停止滚动。
...
改动仍需真机回归。
CI 覆盖十种板型的中英文、CoreS3 视频中英文，以及两种
主题/动画变体，共 24 种配置。这些是构建检查，不代表任何新增板型已经通过真机颜色、
